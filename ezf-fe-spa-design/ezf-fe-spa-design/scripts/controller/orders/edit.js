define(['app', 'model/orders/details', 'utility/messages'], function (app, model, messages) {
    app.controller('EditOrders', ['$scope', '$bus', '$location', 'ngProgress', '$constants', 'toaster', '$rootScope', '$timeout', '$window', '$routeParams',
        function ($scope, $bus, $location, ngProgress, $constants, toaster, $rootScope, $timeout, $window, $routeParams) {
            ...
            $scope.attachEventsForTypeAhead = function () {
                $('html').not("#suggestion-holder, .selectboxhldr span, .suggestion-box a, #order-edit-search-product").click(function () {
                    $scope.suggestions.length = 0;
                    if (!$scope.$$phase) {
                        $scope.$apply();
                    }
            ...
            $scope.draftOrder = function () {
                if ($scope.model.lineItems.length) {
                    if (_.where($scope.model.lineItems, {
                        ...
                        $('#modal-order-edit-expo-error').modal();
                    } else {
                        ngProgress.start();
                        $scope.cleanDataPre();
                    ...
                    $bus.fetch({
                        ...
                        .done(function (success) {
                            if (success.response.success.length) {
                                ...
                                toaster.pop("success", messages.orderUpdateSucess);
                                ...
                                if (errors.length) {
                                    ...
                                    toaster.pop("error", messages.orderUpdateError, "", 0);
                                }
                            }
                            ngProgress.complete();
                        }).fail(function (error) {
